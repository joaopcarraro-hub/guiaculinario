...
> ## ⛔ NÃO use esta skill para foto de receita deste projeto
>
> O Cardápio Gastronômico tem pipeline próprio: **`scripts/gerar-imagens.js`**. Ele carrega as 398
> receitas de `data/*.js`, escolhe arquétipo e louça por regra, monta o prompt, nomeia o arquivo
> pelo slug que o app procura e é idempotente (nunca regera o que já existe).
>
> Esta skill aqui é genérica. Ela não conhece os 4 arquétipos (`prato`, `molho`, `preparo`,
> `processo`), não conhece as 10 louças, não conhece o contrato de nome de arquivo e não conhece as
> decisões de enquadramento — cada uma delas custou uma rodada de geração ou dinheiro para
> descobrir. Usá-la para uma receita produz uma imagem fora do padrão do acervo, com nome de arquivo
> que o app não procura, e **gasta dinheiro real** para isso.
>
> Contrato: `docs/CONTRATO-IMAGENS-REDESIGN.md` · Operação: `docs/COMO-RODAR-IMAGENS.md`
>
> Para foto de receita, o comando é sempre:
> ```
> node scripts/gerar-imagens.js --receita="Nome Exato"          # dry run, custo zero
> node scripts/gerar-imagens.js --gerar --receita="Nome Exato"  # gera
> ```
>
> Esta skill continua válida para o resto: ícone, OG image, ilustração, banner — qualquer imagem
> que **não** seja foto de prato do acervo. Inclusive a imagem dedicada de tile de categoria
> (1:1, 600×600), que por decisão de projeto não sai do pipeline de receitas.
